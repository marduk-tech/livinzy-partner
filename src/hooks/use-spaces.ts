...
export const useFetchSpacesByProject = (projectId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await axiosApiInstance.get(`/spaces/project/${projectId}`);
      return data;
    },
  });
};
...
export const useSaveSpace = () => {
  return useMutation({
    mutationFn: async (spaceData: Space) => {
      let response;
      if (spaceData._id) {
        response = await axiosApiInstance.put(`/spaces/${spaceData._id}`, spaceData);
